...
(function () {
  ...
  async function checkForUpdates(force) {
    if (!detailId) return;
    const now = Date.now();
    if (!force && lastCheckId === detailId && now - lastCheckAt < 60000) {
      paintUpdateAll();
      return;
    }
    const run = ++updateCheckRun;
    try {
      const list = await bridge().checkContentUpdates(detailId);
      if (run !== updateCheckRun || !detailId) return;
      updateMap = new Map((list || []).map((u) => [updateKey(u.category, u.file), u]));
      lastCheckAt = Date.now();
      lastCheckId = detailId;
    } catch {
      if (run !== updateCheckRun || !detailId) return;
    }
  paintUpdateAll();
  ...
  async function updateAllContent() {
    const jobs = [...updateMap.values()].filter((u) => u.updateAvailable && u.latestId && u.projectId);
    ...
    for (const u of jobs) {
      if (!detailId) break;
      try {
        await bridge().switchContentVersion(u.file, u.projectId, u.latestId, detailId, u.category);
        ok += 1;
      } catch {
        failed += 1;
      }
  ...
  function contentCard(m, key) {
    ...
    const info = updateMap.get(updateKey(key, m.file));
    ...
    if (info && info.updateAvailable && info.latestId) {
      ...
      up.addEventListener('click', async () => {
        up.disabled = true;
        try {
          const res = await bridge().switchContentVersion(m.file, projectId, info.latestId, detailId, key);
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (detailBackupBtn) {
      detailBackupBtn.addEventListener('click', async () => {
        ...
        try {
          const res = await bridge().backupInstance(detailId);
